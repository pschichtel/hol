import { HolError, HolInput, HolRequest, HolResponse, HolMetadata, HolFilter, Hol } from "./model"
...
export function hol(request: HolRequest): Promise<HolResponse> {
    ...
    return fetch(request.input, request.init).then(
        response => fetchToHolRequest(response, clonedMetadata),
        error => {
...
export function holToFetch(hol: Hol): (input: HolInput, init?: RequestInit) => Promise<Response> {
    return function HolAsFetch(input, init) {
        return hol(new HolRequest(input, init, new HolMetadata())).then(
