import { HolError, HolFilter, HolMetadata, HolMetadataKey, HolResponse } from "./model"
import { HolResult } from "./result"

export const RetryAttemptKey = new HolMetadataKey<number>("The attempt that got accepted")

export function retry(
    predicate: (result: HolResult, attempt: number) => Promise<boolean>,
): HolFilter {
    return async function RetryFilter(request, execute): Promise<HolResponse> {
        ...
        while (true) {
            try {
                ...
                const retryResult = await predicate(
